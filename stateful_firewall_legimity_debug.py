...
class Firewall(object):
	...
	def verify_legitimite(self):
		...
		for key in l4_fw_rules:
			nw_src=clean_ip(key[srcip])
			nw_dst=clean_ip(key[dstip])
			self.config_protocol_flow(pkt.arp.REQUEST,pkt.ethernet.ARP_TYPE,None,None,IPAddr(key[srcip]),IPAddr(key[dstip]),True)
			self.config_protocol_flow(pkt.arp.REPLY,pkt.ethernet.ARP_TYPE,None,None,IPAddr(key[srcip]),IPAddr(key[dstip]),True)
			self.config_protocol_flow(pkt.ipv4.ICMP_PROTOCOL,pkt.ethernet.IP_TYPE,None,None,IPAddr(key[srcip]),IPAddr(key[dstip]),True)
			self.config_protocol_flow(pkt.ipv4.TCP_PROTOCOL,pkt.ethernet.IP_TYPE,None,None,IPAddr(key[srcip]),IPAddr(key[dstip]),True)
